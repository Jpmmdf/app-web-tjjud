...
4. o `release-please` abre ou atualiza a PR de release por componente
5. a PR de release pode ser aprovada automaticamente no mesmo workflow
6. ao fazer merge da PR de release, o `release-please` cria a tag e a GitHub Release com `GITHUB_TOKEN`
...
- `main-delivery.yml`: executa no `push` da `main`, detecta as areas alteradas, roda a CI relevante, abre ou atualiza PRs de release, autoaprova PRs de release validas e, quando uma release e criada, chama os jobs de publicacao Docker
